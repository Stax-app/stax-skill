...
- **Palette system** (real product UI, from user screenshots): near-black `#0B0E11` ground on every frame; the product's GREEN `#2BD97C` carries every accent — stepper checks, selected chips, metric numerals, chart stroke, deploy banner, CTA. Sell/negative markers are orange `#F97316`; SPY comparison line is dashed gray `#8B93A7`. Headlines near-white `#F2F5F4`; muted `#A8B3AE`. Cards are dark surfaces `#11161A` with subtle green-tinted borders `rgba(43,217,124,0.25)`; metric cards get a soft green border glow. User chat bubble = dark green-tinted rounded bubble with white text, right-aligned; Buddi reply = plain text with a small teal avatar circle + "Buddi" name label.
...
- **Negative list**: no slideshow front-loading (nothing dumped in the first 25%); no screensaver drift (nothing floats independently); no `back.out`/`bounce.out`; no second accent color; no cobalt headlines; no shadows on cards; figures come from the real product screenshots (Return 104.7% vs S&P 500 96.3%, risk-adjusted 1.05, max drawdown 16.0% vs 19.0%, 747 versions tested, tickers PANW/ZTO/ACN/CRNX/DTM/EQT); no phone bezels or browser chrome; atmosphere (concentric rings) on the closing frame only.
...
- voiceover: "I typed 'deploy' — and it paper-trades on real prices. Free. Before I risk a dollar."
- duration: 5.12s
- transition_in: push-slide LEFT
- status: animated
